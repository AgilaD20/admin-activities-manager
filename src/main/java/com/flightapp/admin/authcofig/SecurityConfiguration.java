...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.csrf().disable().headers().frameOptions().sameOrigin().and().formLogin().disable().authorizeRequests()
				.antMatchers("/api/v1.0/flight/airline/authenticate", "/swagger-ui.html/**", "/swagger-resources/** ",
						"/webjars/springfox-swagger-ui/**", "/v2/api-docs", "/spring-security-rest/**")
				.permitAll().antMatchers("/api/v1.0/flight/airline/*").hasAuthority("MANAGE_AIRLINE_ACCESS")
